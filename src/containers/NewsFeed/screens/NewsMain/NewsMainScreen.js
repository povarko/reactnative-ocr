...
class NewsMainScreen extends React.Component {
  ...
  _onPress = index => {
    const data = TICKER_DATA[0].news;
    const newsData = data[index];
    this.props.navigation.navigate("NewsMainExpanded", { data: newsData });
  };
  _onPressComment = index => {
    const data = TICKER_DATA[0].news;
    const newsData = data[index];
    this.props.navigation.navigate("NewsMainExpanded", { data: newsData });
  };
    ...
    return (
      <View style={styles.expanditem}>
        ...
        >
          ...
          <TouchableOpacity
            style={{ marginLeft: 30 }}
            onPress={() => this._onPressComment(index)}
          >
            <View style={{ flexDirection: "row" }}>
